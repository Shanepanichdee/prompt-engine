...
## Brand Style (datashane.com — apply to ALL UI work)

This project shares the datashane.com brand with [QR Code Engine](https://qr-engine.data-shane.com). Always apply these when writing or editing CSS/UI.

**Color palette (light mode):**

| Token | Value | Use |
|-------|-------|-----|
| `--brand` | `#008080` | Primary teal — buttons, active states, focus rings |
| `--brand-2` | `#004C4C` | Dark teal — hover states, text on teal |
| `--brand-soft` | `#D4EEEE` | Light teal — pill backgrounds, active card fill |
| `--bg` | `#EEF7F7` | Page background — solid light teal tint |
| `--surface` | `#ffffff` | Card/panel background |
| `--surface-2` | `#F4FAFA` | Input/secondary surface |
| `--text` | `#002A2A` | Body text — very dark teal |
| `--muted` | `#4A7575` | Secondary text — mid teal |
| `--border` | `#B8DEDE` | Borders — light teal |

**Design rules:**
- Background: solid `#EEF7F7`, never a gradient
- Topbar: flat sticky strip, same bg as page, `1px solid var(--border)` bottom only, no border-radius, no box-shadow
- Primary buttons (CTA): filled teal `var(--brand)`, white text, `border-radius: 999px`
- Secondary/ghost buttons: teal border + teal text on transparent/surface bg
- Cards/panels: white (`var(--surface)`), teal-tinted border `var(--border)`, subtle `border-radius: 12-16px`
- Focus ring: `box-shadow: 0 0 0 3px rgba(0, 128, 128, 0.2)`
- Font stack: `'Plus Jakarta Sans', 'Avenir Next', 'Segoe UI', sans-serif`

**Brand identity text:**
- Product name: "Prompt Engine"
- Attribution line: "by datashane.com" (shown in topbar, footer links)
- Favicon: teal `#008080` square, white "PE" text (generated by `apps/web/app/icon.tsx`)
